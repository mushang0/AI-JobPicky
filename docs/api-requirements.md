...
| API-021 | 上传简历生成画像草稿 | `POST /api/v1/user/profile-imports` | ✅ 已完成 |
...
画像支持直接填写表单，或上传受支持的简历生成可校对草稿。AI 对话完善画像仍属于未来能力，未实现前不提供
可点击入口、占位接口或伪造结果。

三种录入方式都必须汇入同一份可校对画像草稿：

```text
手工表单 ─────────┐
简历文件解析 ─────┼→ 统一画像草稿 → 用户检查和修正 → 不可变画像快照 → 新推荐任务
AI 对话（未来） ──┘
```

简历解析或 AI 对话不得直接覆盖正式画像，也不得自动发起推荐。
...
- 必须登录，用户身份来自 Access Token；请求不接受 `user_id`。
- multipart 只包含一个必填字段 `file`，不扩展 API-013 接受文件。
- 首版同步执行上传校验、按格式准备模型输入和结构化模型解析，成功返回 HTTP `200`；不创建导入运行或轮询接口。
- 只有真实延迟、恢复或并发数据证明必要时，才升级为异步导入任务。
- 调用不会保存原文件、创建画像版本、修改当前画像、扣积分或发起推荐。
...
| 单文件大小 | 最大 10 MiB |
| PDF 页数 | 最大 4 页 |
| 非 PDF 送入模型的文本 | 最大 50000 字符；超出时截取并返回警告 |

- PDF 按页渲染为临时 PNG 图片后发送给支持视觉输入的模型，不要求 PDF 包含可提取文本，因此支持扫描页和图片型简历；
  加密 PDF 和旧版 `.doc` 不在首版范围。
- PDF 页面图片与推荐共用 `JOBPICKY_LLM_MODEL`；该模型必须支持视觉输入。未配置模型时返回 `503 DEPENDENCY_UNAVAILABLE`，不把页面图片发送给其他文本模型。
- PDF 页面图片只在当前请求内生成和读取，不写入数据库、对象存储、日志或普通画像响应；模型供应商请求仍按敏感简历输入处理。
- DOCX 使用文档正文中的可见文本；TXT/Markdown 接受 UTF-8 或 GB18030 文本。
- 文件扩展名、格式结构、空内容、PDF 渲染结果和文本提取结果均由服务端校验，不能只依赖前端 `accept`。
- 原始文件只在当前请求内读取，不写入数据库、对象存储、日志或普通画像响应。
...
```json
{
  "draft": {
    ...
    "experience_summary": "参与后端接口、数据建模和异步任务开发。",
    "excluded_roles": [],
    "extra_request": null
  },
  "warnings": ["未从简历中确认目标城市和期望薪资，请补充。"]
}
```

`ProfileImportDraft` 使用与画像表单相同的字段和规范化枚举，但在导入阶段允许 `target_roles=[]`，也允许
`skills=[]` 且 `experience_summary=null`。这是为了让无法确认的信息保持缺失，而不是要求模型编造内容来通过
正式画像校验。用户通过 API-013 保存前仍必须满足 §15.4 的全部约束。

模型必须把上传文本或 PDF 页面图片视为不可信数据，只提取或概括其中可验证的信息：

- 不把简历内的指令当作系统指令；
- 不返回姓名、手机号、邮箱、证件号、照片、住址等与匹配画像无关的敏感信息；
- 不补写不存在的工作、项目、教育、技能或偏好；
- 无法确认的字段保持空值，并在 `warnings` 中用简体中文提示用户校对；
- 若根据最近职位或项目方向给出目标岗位建议，必须有文字或页面内容依据并明确提示这是待确认建议。
...
|---:|---|---|
| `401` | `AUTHENTICATION_REQUIRED` | 未登录 |
| `413` | `VALIDATION_ERROR` | 文件超过大小限制 |
| `415` | `VALIDATION_ERROR` | 扩展名或文件格式不支持 |
| `422` | `PROFILE_PARSE_FAILED` | 文件损坏、加密、PDF 无法渲染、页数超过 4 页或内容无法形成草稿 |
| `502` | `PROFILE_PARSE_FAILED` | 模型输出未通过本地结构校验 |
| `503` | `DEPENDENCY_UNAVAILABLE` | 模型未配置、超时或供应商暂时不可用 |

错误响应不回显文件内容、联系方式、模型原始输出或供应商堆栈。

### 18.5 前端入口与交互

- 在现有“我的求职画像”页面顶部显示“上传简历自动生成草稿”，同时保留原手工填写入口。
- 使用原生文件选择器限制可选后缀；上传中禁用重复提交并显示真实等待状态。
- 解析成功后把 `draft` 填入现有分步表单，保留当前画像的 `base_version`，显示 `warnings` 和“尚未保存”提示，
  从第一步开始由用户逐项检查。
- 解析失败时保留当前画像和未保存表单，不展示伪造草稿；PDF 渲染和视觉模型失败也使用同一错误语义。
- 用户仍须明确点击“保存画像”调用 API-013；上传成功本身不能显示为画像已保存。

### 18.6 未来画像录入方式

未来 AI 对话只修改独立草稿，并在页面同步展示字段变化。AI 必须基于用户输入，不得编造经历；只有用户
确认并调用 API-013 后才生成正式画像版本。旧版 DOC 与异步导入在真实样本或运行数据
证明需要时再确认契约，现在不创建占位接口。
...
10. 支持格式的简历上传会生成可校对草稿；上传本身不修改当前画像、不扣积分、不自动推荐。
11. 不支持、过大、损坏、加密或无法渲染的文件显式失败，失败后保留当前页面内容。
12. 模型非法结构和无依据内容不能进入正式画像；原始简历与无关个人敏感信息不会进入日志或普通响应。
13. 页面没有可操作的 AI 对话、旧版 DOC 或异步任务假入口。
...
- 简历上传入口使用 API-021 并回填现有画像表单；AI 对话入口继续隐藏，直到对应能力真实实现。
...
| 简历文件大小 | `10 MiB` | API-021 单文件上限 |
| PDF 页数 | `4` | API-021 页面渲染上限 |
| 简历解析文本 | `50000` 字符 | 超出时截取并在响应中警告 |
...
除 API-021 的上传保护限制在首版固定外，这些值允许通过后端配置调整，但调整后的真实限制必须由 API 响应
或 OpenAPI 表达；生产积分、余额上限和岗位池规模在上线前根据运营与容量重新确认。只有真实简历样本证明
需要调整时，再把上传限制配置化并同步 OpenAPI。
...
| 位置 | 原冲突 | 已同步为 | 状态 |
|---|---|---|---|
| `docs/requirements.md` | 仍以简历文本/文件创建画像，且把具体前端 UI 放在 Later | 手工画像表单及简历草稿导入、登录、岗位池、推荐和收藏页面成为当前首版范围 | ✅ 已完成 |
| `docs/architecture.md` §4.6、§5.3、§6 | 画像仍是 `POST + PATCH`，推荐请求仍由前端传 `profile_id`，没有认证、收藏列表和筛选选项契约 | 同步 API-001～021、最新 DTO、自动使用当前画像和可选/必需身份边界 | ✅ 已完成 |
...
| `tests/` 与当前 OpenAPI | 只覆盖已有底座，未验证本文契约 | 为 API-001～021 增加契约、权限、幂等、并发、退款、上传和安全测试并生成 OpenAPI | ✅ 已完成 |
