...
- 首版同步执行上传校验、文本提取和结构化大模型解析，成功返回 HTTP `200`；不创建导入运行或轮询接口。
...
| 单文件大小 | 最大 10 MiB |
| PDF 页数 | 最大 50 页 |
| 送入模型的文本 | 最大 50000 字符；超出时截取并返回警告 |

- PDF 必须包含可提取文本；扫描件 OCR、图片简历、加密 PDF 和旧版 `.doc` 不在首版范围。
- DOCX 使用文档正文中的可见文本；TXT/Markdown 接受 UTF-8 或 GB18030 文本。
- 文件扩展名、格式结构、空内容和文本提取结果均由服务端校验，不能只依赖前端 `accept`。
...
| `422` | `PROFILE_PARSE_FAILED` | 文件损坏、加密、无可提取文本或内容无法形成草稿 |
...
11. 不支持、过大、损坏、加密或无可提取文本的文件显式失败，失败后保留当前页面内容。
12. 模型非法结构和无依据内容不能进入正式画像；原始简历与无关个人敏感信息不会进入日志或普通响应。
13. 页面没有可操作的 AI 对话、OCR、旧版 DOC 或异步任务假入口。
